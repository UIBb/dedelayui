...
if ($dopost == "ok") {
    ...
    if ($step == -1) {
        if ($uparc == 0) sleep(1);
        layerMsg("成功更新所有缓存！", "-1", 1, 1000, 1);
        exit();
    } //更新栏目缓存
    else if ($step == 1) {
        UpDateCatCache();
        ClearOptCache();
        layerMsg("成功更新栏目缓存，及后台栏目选项,准备更新枚举缓存...", "sys_cache_up.php?dopost=ok&step=2&uparc=$uparc", 1, 1000, 1);
        exit();
    } //更新枚举缓存
    else if ($step == 2) {
        ...
        WriteEnumsCache();
        //WriteAreaCache(); 已过期
        layerMsg("成功更新枚举缓存，准备更新调用缓存...", "sys_cache_up.php?dopost=ok&step=3&uparc=$uparc", 1, 1000, 1);
        exit();
        ...
        layerMsg('修正错误文档操作已经取消，请在&lt;系统-&gt;系统错误修复[S]&gt;中操作...', 'sys_cache_up.php?dopost=ok&step=-1&uparc=1', 1, 5000, 1);
        exit();
    }
}
