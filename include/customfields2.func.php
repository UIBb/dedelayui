...
function GetFormItem($ctag, $admintype='admin')
{
    ...
    $formitem = $formitem = GetSysTemplets("custom_fields_{$admintype}2.htm");
    ...
    if($fieldType=='select')
    {
        ...
        $myformItem = "<div class='layui-input-inline'><select name='$fieldname'>";
        foreach($items as $v)
        {
            $v = trim($v);
            if($v!='') {
        ...
        $myformItem .= "</select></div>\r\n";
        $innertext = $myformItem;
    }
    else if($fieldType=='stepselect')
    {
            global $hasSetEnumJs,$cfg_cmspath;
            $cmspath = ( (empty($cfg_cmspath) || !preg_match('/[/$]/', $cfg_cmspath)) ? $cfg_cmspath.'/' : $cfg_cmspath );//2011.08.04 根据论坛反馈修正联动模型 （by：简致的鱼）
            $myformItem = '';
            ...
            {
                $myformItem .= '<script language="javascript" type="text/javascript" src="'.$cmspath.'images/enums2.js"></script>'."\r\n";
    ...
    else if($fieldType=='radio')
    {
        ...
        foreach($items as $v)
        {
            ...
            if($v!='')
            {
                $myformItem .= ($i==0 ? "<input type='radio' name='$fieldname' class='np' value='$v' title='$v' checked>\r\n" : "<input type='radio' name='$fieldname' class='np' value='$v' title='$v'>\r\n");
                $i++;
            }
        }
        $innertext = $myformItem;
    ...
    else if($fieldType=='checkbox')
    {
        ...
        foreach($items as $v)
        {
            ...
            if($v!='')
            {
                ...
                {
                    $myformItem .= "<label><input type='checkbox' name='{$fieldname}[]' class='np' value='$v' title='$v'></label>\r\n";
                } else {
          $myformItem .= "<input type='checkbox' name='{$fieldname}[]' class='np' value='$v' title='$v'>\r\n";
                }
    ...
    {
        $innertext = "<textarea name='$fieldname' id='$fieldname' class='layui-textarea'></textarea>\r\n";
    }
    else if($fieldType=="datetime")
    {
        $nowtime = GetDateTimeMk(time());
        $innertext = "<div class='layui-input-inline'><input name=\"$fieldname\" value=\"$nowtime\" type=\"text\" id=\"$fieldname\" class=\"layui-input\" onclick=\"layui.laydate({elem: this, istime: true, format: 'YYYY-MM-DD hh:mm:ss'})\"  /></div>";
    }
    else if($fieldType=='img'||$fieldType=='imgfile')
    {
        if($admintype=='diy') {
            $innertext = "<input type='file' name='$fieldname' id='$fieldname' style='float:left;margin-right:-2px;width:300px;' />\r\n";
        }
        else {
    ...
    else if($fieldType=='media')
    {
        ...
        {
            $innertext = "<input type='text' name='$fieldname' id='$fieldname' class='layui-input' style='float:left;margin-right:-2px;width:300px;' /> <input name='".$fieldname."_bt' type='button' class='layui-btn' value='浏览...' onClick=\"SelectMedia('form1.$fieldname')\" />\r\n";
        }
    }
    else if($fieldType=='addon')
    {
        ...
        {
            $innertext = "<input type='text' name='$fieldname' id='$fieldname' class='layui-input' style='float:left;margin-right:-2px;width:300px;' /> <input name='".$fieldname."_bt' type='button' class='layui-btn' value='浏览...' onClick=\"SelectSoft('form1.$fieldname')\" />\r\n";
        }
    }
    else if($fieldType=='int'||$fieldType=='float')
    {
        $dfvalue = ($ctag->GetAtt('default')!='' ? $ctag->GetAtt('default') : '0');
		$innertext = "<div class='layui-input-inline'><input type='text' name='$fieldname' id='$fieldname' class='layui-input' value='$dfvalue' /></div> <div class='layui-form-mid layui-word-aux'>(填写数值)</div>\r\n";
    }
    else
    {
        $dfvalue = ($ctag->GetAtt('default')!='' ? $ctag->GetAtt('default') : '');
        $innertext = "<input type='text' name='$fieldname' id='$fieldname' class='layui-input' value='$dfvalue' />\r\n";
    }
    $formitem = str_replace("~name~",$ctag->GetAtt('itemname'),$formitem);
...
function GetFormItemValue($ctag, $fvalue, $admintype='admin', $fieldname='')
{
    ...
    if($ftype=='select')
    {
        $myformItem = "<select name='$fieldname'>";
        if(is_array($items))
        {
            foreach($items as $v)
            {
    ...
    else if($ftype=='radio')
    {
        if(is_array($items))
        {
            foreach($items as $v)
            {
                $v = trim($v);
                if($v=='') continue;
                $myformItem.= ($fvalue==$v ? "<input type='radio' name='$fieldname' class='np' value='$v' checked='checked' title='$v' />\r\n" : "<input type='radio' name='$fieldname' class='np' value='$v' title='$v' />\r\n");
            }
        }
        $innertext = $myformItem;
    }
    ...
    else if($ftype=='checkbox')
    {
        ...
        if(is_array($items))
        {
            foreach($items as $v)
            {
                ...
                {
                    $myformItem .= "<input type='checkbox' name='{$fieldname}[]' class='np' value='$v' checked='checked' title='$v' />\r\n";
                }
                else
                {
                    $myformItem .= "<input type='checkbox' name='{$fieldname}[]' class='np' value='$v' title='$v' />\r\n";
    ...
    {
        $innertext = "<textarea name='$fieldname' id='$fieldname' class='layui-textarea'>$fvalue</textarea>\r\n";
    }
    ...
    {
        $innertext = "<input type='text' name='$fieldname' value='$fvalue' id='$fieldname' style='float:left;margin-right:-2px;width:300px;'  class='layui-input' /> <input name='".$fieldname."_bt' class='layui-btn' type='button' value='浏览...' onClick=\"SelectMedia('form1.$fieldname')\" />\r\n";
    }
    else if($ftype=="addon")
    {
        $innertext = "<input type='text' name='$fieldname' id='$fieldname' value='$fvalue' style='float:left;margin-right:-2px;width:300px;'  class='layui-input' /> <input name='".$fieldname."_bt' class='layui-btn' type='button' value='浏览...' onClick=\"SelectSoft('form1.$fieldname')\" />\r\n";
    }
    else if($ftype=="int"||$ftype=="float")
    {
        $innertext = "<div class='layui-input-inline'><input type='text' name='$fieldname' id='$fieldname'  class='layui-input' value='$fvalue' /></div> <div class='layui-form-mid layui-word-aux'>(填写数值)</div>\r\n";
    }
    else
    {
        $innertext = "<input type='text' name='$fieldname' id='$fieldname'  class='layui-input' value='$fvalue' />\r\n";
    }
    $formitem = str_replace('~name~',$ctag->GetAtt('itemname'),$formitem);
    $formitem = str_replace('~form~',$innertext,$formitem);
    return $formitem;
