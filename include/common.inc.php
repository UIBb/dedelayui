...
if (file_exists(DEDEDATA . '/safe/inc_safe_config.php')) {
    ...
    if (!empty($safe_faqs))
        // $safe_faqs = preg_replace_callback('!s:(\d+):"(.*?)";!se', "'s:'.strlen('$2').':\"$2\";'", $safe_faqs );
        $safe_faqs = preg_replace_callback('#s:(\d+):"(.*?)";#s', function ($match) {
...
$cfg_softname = '内容管理系统';
$cfg_soft_enname = 'JIANZHICMS';
$cfg_soft_devteam = '官方团队';
//文档的默认命名规则
